#!/usr/bin/env python3
"""
V6 — V1 (SDPA) + MPS FlashAttention extreme-sequence path.

Same as V1 for every normal configuration. When seq_len is too large for the
naive baseline (O(S^2) attention matrix impossible), V6 switches the whole
model to bfloat16 so that `F.scaled_dot_product_attention` dispatches to the
MPS FlashAttention kernel — which streams the attention computation in tiles
and never materializes the S x S matrix.
...
def run_extreme_path(
    ...
    unmaterializable). Run the V1 SDPA model in bfloat16 so that
    `F.scaled_dot_product_attention` dispatches to the MPS FlashAttention
    kernel: tiled, streaming attention that never materializes S x S.

    Two stages:
      1. Correctness on a DOWNSCALED sequence (seq=256): bf16 flash SDPA
         vs the exact fp32 baseline (loose tolerance = bf16 precision).
      2. The REAL config in bf16: runnability + timing + peak memory.
         The baseline cannot run, so speedup is reported as N/A.
    """
    sanity_seq = min(config.seq_len, 256)

    print("=== V6 extreme-seq mode (MPS FlashAttention) ===")
    print(f"baseline cannot run seq_len={config.seq_len} "
          f"(SxS attention matrix is unmaterializable)")
    print("V6 runs the whole model in bfloat16 -> SDPA dispatches to the "
          "MPS FlashAttention kernel (tiled, O(S) memory attention)")

    # ---- 1. Downscaled correctness: bf16 flash vs fp32 baseline ----
    ...
    # ---- 2. Real config in bf16 ----
    print(f"\nreal forward: batch={config.batch_size} seq={config.seq_len} "
          f"d_model={config.d_model} heads={config.num_heads} "
          f"layers={config.num_layers} dtype=bfloat16 (flash kernel)")
    torch.manual_seed(args.seed + 100000)
    model = UserOptimizedTransformer(config).to(
        device=device, dtype=torch.bfloat16).eval()

    with torch.inference_mode():
        # No padding mask in extreme mode: keep the pure causal fast path
        # (is_causal=True), which is the only path the flash kernel takes.
        x = torch.randn(
    ...
    print(f"RUNS: YES | median={timing.median_ms:.4f} ms | "
