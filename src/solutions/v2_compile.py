...
class UserOptimizedTransformer(BaselineTransformer):
    """
    V2 — torch.compile the forward.

    Idea: use torch.compile (a.k.a. TorchInductor) to compile the forward
    into a single optimized graph. Inductor can fuse pointwise ops with
    the surrounding matmuls, generate better kernels, and reduce Python
    overhead from per-op dispatch. On a GPU this typically gives a
    1.2-1.5x speed-up; on MPS the wins tend to be smaller because the
    Inductor backend for Metal is less mature than CUDA.

    We compile lazily on the first forward call so the cost of tracing
    is paid during the benchmark's warmup phase and does not pollute the
    timed iterations.
    """

    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config)
        # Lazy: compile on first forward so weight copy / .to() / .eval()
        # are all done before the trace.
        self._compiled_forward = None

    def _baseline_forward(
        self, x: torch.Tensor, valid_token_mask: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        # Bind the parent forward via the class to avoid the recursion
        # of going through nn.Module.__call__ machinery.
