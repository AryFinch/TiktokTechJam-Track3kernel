...
class UserOptimizedTransformer(BaselineTransformer):
    """
    V1.1 — SDPA (V1) + micro-batching for extra-large inputs.

    Idea: keep the V1 SDPA attention (never materializes the S x S
    matrix), and additionally split the BATCH dimension into
    micro-batches when the input is very large. Two reasons:

      1. MPS/Metal graph rejects any single tensor with more than
         2^31 (INT_MAX) elements — at batch=32, seq=100000, d_model=1024
         the input alone is 3.3e9 elements and a monolithic forward
         cannot even be scheduled.
      2. Peak activation memory of one full-batch forward (Q/K/V, FFN
         intermediates, residuals for every sample at once) exceeds the
         unified-memory budget even when attention itself is streaming.

    Micro-batching is mathematically EXACT: samples are independent
    across the batch dimension, so running them in groups and
    concatenating the outputs is identical to one big forward.

    The rest of the transformer (LayerNorm, residual, FFN, GELU) is
    identical to the baseline so the comparison isolates these two
    changes. Parameter names are kept identical so weight copy works.
    """

    # Max elements per micro-batch. Chosen well under the 2^31 MPSGraph
    # tensor limit and small enough that one micro-batch's activations
    # (input + Q/K/V + FFN intermediates) fit comfortably in memory.
    MICRO_ELEM_BUDGET = 1 << 28
    ...
    def forward(
        ...
        batch = x.shape[0]
        elems_per_sample = x.numel() // batch
        micro = max(1, self.MICRO_ELEM_BUDGET // elems_per_sample)
        if micro >= batch:
            # Small enough — run the whole batch in one call (V1 path).
            return super().forward(x, valid_token_mask)

        # Split the batch into micro-batches, run each through the
        # standard V1 SDPA path, and write into one preallocated output
        # tensor (avoids the 2x peak of list + torch.cat). dim-0 slices
        # stay contiguous, so every tensor handed to MPS is small and
        # valid.
        ...
        for start in range(0, batch, micro):
            ...
            xb = x[start:stop]
            mb = (
                valid_token_mask[start:stop]
                if valid_token_mask is not None
                else None
            )
            out[start:stop] = super().forward(xb, mb)
        return out
