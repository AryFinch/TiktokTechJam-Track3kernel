#!/usr/bin/env python3
"""
V7 — V1 (SDPA) + MPS FlashAttention + gradient checkpointing for extreme
sequences.

Same as V1 for every normal configuration. When seq_len is too large for the
naive baseline (O(S^2) attention matrix impossible), V7:
  1. runs the model in bfloat16 so `F.scaled_dot_product_attention`
     dispatches to the MPS FlashAttention kernel (tiled, O(S) attention), and
  2. wraps every transformer layer in `torch.utils.checkpoint` so only the
     layer-boundary activations are kept for backward — each layer's
     internals (Q/K/V, FFN intermediates) are recomputed on demand instead
     of stored.
...
class UserOptimizedTransformer(BaselineTransformer):
    """
    V1 — Use torch.nn.functional.scaled_dot_product_attention (SDPA).

    Idea: replace the explicit QK^T -> mask -> softmax -> @V pipeline with
    a single fused SDPA call. The kernel backend (FlashAttention /
    memory-efficient / math) is selected automatically by PyTorch and
    typically gives a large speed-up, especially for long sequences,
    because it avoids materializing the full S x S attention matrix.

    The rest of the transformer (LayerNorm, residual, FFN, GELU) is
    identical to the baseline so the comparison isolates the attention
    change. Parameter names are kept identical so weight copy works.
    """

    def __init__(self, config: TransformerConfig) -> None:
        # Build with baseline structure (so layers/norms/FFN exist).
        super().__init__(config)
        # Swap in the SDPA attention module on every block. The new module
        # exposes the same parameter names (q_proj, k_proj, v_proj, out_proj)
        # so copy_model_weights() will fill them in correctly.
        for layer in self.layers:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-extreme-path",
        action="store_true",
        help="disable the bf16 MPS FlashAttention extreme-seq path "
             "(run the normal baseline-vs-optimized comparison instead)",
    )
...
def run_extreme_path(
    ...
    """
    Sequence too long for the naive baseline (S x S attention is
    unmaterializable). V7 combines two memory techniques:

      1. bfloat16 + SDPA -> MPS FlashAttention kernel (tiled, O(S)
         attention memory — never materializes the S x S matrix);
      2. GRADIENT CHECKPOINTING: each layer is wrapped in
         torch.utils.checkpoint, so the autograd graph keeps only the
         layer-boundary activations and recomputes each layer's internals
         (Q/K/V, FFN intermediates) during backward.

    Stages:
      1. Correctness on a DOWNSCALED sequence (seq=256):
         checkpointed forward == plain forward (exact), and bf16 flash
         vs the exact fp32 baseline (loose tolerance = bf16 precision).
      2. The REAL config in bf16, grad-mode forward + backward with
         per-layer checkpointing: runnability + timing + peak memory.
         The baseline cannot run, so speedup is reported as N/A.
    """
    from torch.utils.checkpoint import checkpoint as grad_checkpoint

    sanity_seq = min(config.seq_len, 256)

    print("=== V7 extreme-seq mode (MPS FlashAttention + grad ckpt) ===")
    print(f"baseline cannot run seq_len={config.seq_len} "
          f"(SxS attention matrix is unmaterializable)")
    print("V7 = bfloat16 SDPA (MPS FlashAttention kernel, O(S) attention "
          "memory) + per-layer gradient checkpointing (recompute layer "
          "internals in backward instead of storing them)")

    # ---- 1. Downscaled correctness ----
    ...
    baseline = BaselineTransformer(small_config).to(
        device=device, dtype=torch.float32).eval()
    optimized = UserOptimizedTransformer(small_config).to(
    ...
    with torch.inference_mode():
        ...
        candidate = optimized(x.to(torch.bfloat16),
                              valid_mask).float()
        result = compare_outputs(reference, candidate, rtol=0.1, atol=0.05)
        print(f"bf16 flash vs fp32 baseline (seq={sanity_seq}): "
              f"{'PASS' if result.passed else 'FAIL'} | "
              f"max_abs={result.max_abs_error:.6g} | "
              f"failed={result.failed_elements}/{result.total_elements}")

    # Checkpointed forward must be bitwise-identical to the plain forward
    # (checkpointing only changes WHAT IS STORED, not the math).
    with torch.no_grad():
        xb = x.to(torch.bfloat16)
        plain = optimized(xb, valid_mask)
        ckpt_out = xb
        for layer in optimized.layers:
            ckpt_out = grad_checkpoint(
                layer, ckpt_out, valid_mask, config.causal,
                use_reentrant=False,
            )
        ckpt_out = optimized.final_norm(ckpt_out)
        if valid_mask is not None:
            ckpt_out = ckpt_out.masked_fill(~valid_mask[..., None], 0)
        same = torch.equal(plain, ckpt_out)
        print(f"checkpointed vs plain forward (seq={sanity_seq}): "
              f"{'EXACT' if same else 'DIFFERS'}")

    # Free the sanity-check models before the big allocation.
    del baseline, optimized, x, valid_mask, reference, candidate
    del plain, ckpt_out
    if device.type == "mps":
        torch.mps.empty_cache()

    # ---- 2. Real config: bf16 flash + grad-mode + checkpointing ----
    print(f"\nreal fwd+bwd: batch={config.batch_size} seq={config.seq_len} "
          f"d_model={config.d_model} heads={config.num_heads} "
          f"layers={config.num_layers} dtype=bfloat16 "
          f"(flash kernel + checkpoint)")
    torch.manual_seed(args.seed + 100000)
    model = UserOptimizedTransformer(config).to(
        device=device, dtype=torch.bfloat16).eval()

    # Grad mode (NOT inference_mode): gradient checkpointing only saves
    # memory when autograd is active — that is exactly its purpose.
    x = torch.randn(
    ...
    def fwd_bwd() -> None:
        ...
        for layer in model.layers:
            out = grad_checkpoint(
                layer, out, None, config.causal, use_reentrant=False,
            )
    ...
    for _ in range(repeats):
        if device.type == "mps":
            torch.mps.synchronize()  # MPS is async: sync before timing
        start = time.perf_counter_ns()
        fwd_bwd()
        if device.type == "mps":
            torch.mps.synchronize()  # and after, so we time real compute
    ...
    timing = TimingResult(samples)
    tokens_per_call = config.batch_size * config.seq_len
    throughput = tokens_per_call * 1000.0 / timing.median_ms
    print(f"RUNS: YES | fwd+bwd median={timing.median_ms:.4f} ms | "
          f"mean={timing.mean_ms:.4f} ms | p90={timing.p90_ms:.4f} ms | "
          f"throughput={throughput:.2f} token/s (fwd+bwd) | "
          f"mps-allocated={peak_mem_gb:.2f} GiB")
    print("speedup  : N/A (baseline unrunnable — it would need an O(S^2) "
          "attention matrix)")
    print("note     : checkpointing trades ~1 extra layer recompute per "
          "backward for O(1 layer) activation memory instead of O(L)")
    return 0
...
def main() -> int:
    ...
    # Extreme sequence: the naive baseline cannot run (O(S^2) attention).
    # Switch to the bf16 MPS FlashAttention path instead.
    if (
        device.type == "mps"
        and config.seq_len >= EXTREME_SEQ_THRESHOLD
        and not args.no_extreme_path
    ):
