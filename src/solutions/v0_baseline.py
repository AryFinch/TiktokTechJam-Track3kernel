...
class UserOptimizedTransformer(BaselineTransformer):
    ...
    The forward just delegates to the parent BaselineTransformer, which uses
    three separate Q/K/V linear projections, an explicit fp32 softmax, and
    a hand-rolled multi-head attention. We keep this copy around so the
    other variants have a clean reference number on the same machine
    ...
    def forward(
        ...
        # No optimization — just call the baseline path.
