...
class SDPASelfAttention(nn.Module):
    ...
    torch.nn.functional.scaled_dot_product_attention. Same parameter
    names as the baseline so weight copy works without changes.
    """
    ...
    def forward(
        ...
        # On MPS, SDPA rejects is_causal=True together with attn_mask, so
        # when both are needed we fuse them into one additive mask.
...
class UserOptimizedTransformer(BaselineTransformer):
    ...
    Use scaled_dot_product_attention for the attention block AND wrap the
    whole forward with torch.compile. The two optimizations target
    different costs:
      * SDPA removes the materialised S x S attention matrix and picks
        the best attention kernel (FlashAttention when available).
      * torch.compile fuses the surrounding pointwise ops (LayerNorm,
        residual add, GELU, projections) and removes Python dispatch
        overhead inside each block.
    Combined, this is typically the fastest configuration that does
    not require a custom CUDA/Triton kernel.
    """

    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config)
        # Replace each block's attention with the SDPA version. Parameter
        # names are unchanged so load_state_dict works.
        for layer in self.layers:
            layer.attention = SDPASelfAttention(config.d_model, config.num_heads)
        # Lazy compile; the first forward (during warmup) pays the trace cost.
