...
class UserOptimizedTransformer(BaselineTransformer):
    """
    V1.2 — SDPA (V1) + automatic low precision for extra-large inputs.

    Idea: keep the V1 SDPA attention (never materializes the S x S
    matrix), and additionally run the forward internally in bfloat16
    when the input is extra-large. Rationale:

      * bf16 halves every activation tensor (input copy, Q/K/V, FFN
        intermediates, residuals), so monolithic forwards that OOM in
        fp32 fit in the unified-memory budget.
      * bf16 is the dtype the MPS FlashAttention kernel is fastest in,
        so long-sequence attention also gets faster, not just smaller.

    Precision cost: bf16 has ~8 bits of mantissa, so outputs deviate
    from the fp32 baseline by ~1e-2 relative — an accepted trade-off
    for the "too big seq" regime where fp32 simply cannot run at all.

    Normal-sized inputs (below the threshold) take the exact fp32 V1
    path, so accuracy on regular configurations is unaffected.
    ...
    # Only engage the low-precision path for extra-large inputs; below
    # this the exact fp32 V1 path is used.
    LOW_PRECISION_ELEM_THRESHOLD = 1 << 28
    ...
    def forward(
        ...
        if (
            x.numel() <= self.LOW_PRECISION_ELEM_THRESHOLD
            or x.dtype is not torch.float32
            or x.device.type == "cpu"
        ):
            # Normal-sized input (or already low precision): exact V1 path.
            return super().forward(x, valid_token_mask)

        # Extra-large input: run the whole forward under bf16 autocast and
        # cast the result back to the caller's dtype.
        with torch.autocast(
            device_type=x.device.type, dtype=self.LOW_PRECISION_DTYPE
        ):
